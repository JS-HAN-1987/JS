...
static const uint16_t ILI932x_regValues[] PROGMEM = {
  ...
  ILI932X_RGB_DISP_IF_CTRL1, 0x0, // [X] RIM1=0, RIM0=1 => 16-bit mode
  ILI932X_FRM_MARKER_POS   , 0x0,
  ILI932X_RGB_DISP_IF_CTRL2, 0x0,
  ILI932X_POW_CTRL1        , 0x0000, // [X] AP2=0, AP1=0, AP0=1 => Gamma driver amplifier 1.00, 1.00
...
#endif
#if 0 // http://www.mikroe.com/forum/viewtopic.php?f=97&t=57408
  ILI932X_GAMMA_CTRL1      , 0x0000,
  ILI932X_GAMMA_CTRL2      , 0x0707,
  ILI932X_GAMMA_CTRL3      , 0x0307,
  ILI932X_GAMMA_CTRL4      , 0x0200,
  ILI932X_GAMMA_CTRL5      , 0x0008,
  ILI932X_GAMMA_CTRL6      , 0x0004,
  ILI932X_GAMMA_CTRL7      , 0x0000,
  ILI932X_GAMMA_CTRL8      , 0x0707,
  ILI932X_GAMMA_CTRL9      , 0x0002,
  ILI932X_GAMMA_CTRL10     , 0x1d04,
#endif
#if 1 // http://www.mikroe.com/forum/viewtopic.php?f=97&t=57408 #2
  ILI932X_GAMMA_CTRL1      , 0x0000,
  ILI932X_GAMMA_CTRL2      , 0x0707,
  ILI932X_GAMMA_CTRL3      , 0x0307,
  ILI932X_GAMMA_CTRL4      , 0x0200,
  ILI932X_GAMMA_CTRL5      , 0x0207,
  ILI932X_GAMMA_CTRL6      , 0x0306,
  ILI932X_GAMMA_CTRL7      , 0x0102,
  ILI932X_GAMMA_CTRL8      , 0x0707,
  ILI932X_GAMMA_CTRL9      , 0x0702,
  ILI932X_GAMMA_CTRL10     , 0x1604,
#endif
#if 0 // stm32plus
  ILI932X_GAMMA_CTRL1      , 0x0006,
  ILI932X_GAMMA_CTRL2      , 0x0101,
  ILI932X_GAMMA_CTRL3      , 0x0003,
  ILI932X_GAMMA_CTRL4      , 0x0106,
  ILI932X_GAMMA_CTRL5      , 0x0b02,
  ILI932X_GAMMA_CTRL6      , 0x0302,
  ILI932X_GAMMA_CTRL7      , 0x0707,
  ILI932X_GAMMA_CTRL8      , 0x0007,
  ILI932X_GAMMA_CTRL9      , 0x0600,
  ILI932X_GAMMA_CTRL10     , 0x020b,
#endif
...
void Adafruit_TFTLCD::on(void) {
	CS_ACTIVE;
	if (driver == ID_932X) {
		writeRegister16(ILI932X_DISP_CTRL1, 0x0133); /*  Display Control  DISPLAY ON */
	} else {
		writeRegister16(0x07,0x0012);
		delay(40);
		writeRegister16(0x07,0x0017); /*  Display Control  DISPLAY ON */
...
void Adafruit_TFTLCD::off(void) {
	CS_ACTIVE;
	if (driver == ID_932X) {
		writeRegister16(0x07,0x0000);
	} else {
		writeRegister16(0x07,0x0012);
		delay(40);
		writeRegister16(0x07,0x0000); /*  Display Control  DISPLAY OFF */
	}
	CS_IDLE;
};
...
uint16_t Adafruit_TFTLCD::readPixel(int16_t x, int16_t y) {
  ...
  if((driver == ID_932X) || (driver ==ID_S6D0154) || (driver == ID_ILI9327)) {
    ...
    // tried adjusting timing in _read8() etc. to no avail.
    for(uint8_t pass=0; pass<2; pass++) {
      ...
      _read8(hi);     // First 2 bytes back are a dummy read
      _read8(hi);
      _read8(hi);     // Bytes 3, 4 are actual pixel value
      _read8(lo);
    ...
    setReadDir();  // Set up LCD data port(s) for READ operations
    CD_DATA;
    _read8(r);      // First byte back is a dummy read
    _read8(r);
    _read8(g);
    _read8(b);
    setWriteDir(); // Restore LCD data port(s) to WRITE configuration
...
#ifdef read8isFunctionalized
uint8_t Adafruit_TFTLCD::_read8fn(void) {
  uint8_t result;
  _read8inline(result);
  return result;
}
