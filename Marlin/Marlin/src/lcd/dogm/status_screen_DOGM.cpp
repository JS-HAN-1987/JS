...
*/
    ...
    //if (PAGE_UNDER(7)) 
	{
      ...
      #endif
      if (dodraw) 
		  _draw_centered_temp(target + 0.5, tx, 0);
    }

    //if (PAGE_CONTAINS(28 - INFO_FONT_ASCENT, 28 - 1))
    _draw_centered_temp(temp + 0.5f, tx, STATUS_HEATERS_Y+ STATUS_HEATERS_HEIGHT);
...
void MarlinUI::draw_status_screen() {
  ...
  static char xstring[5
    ...
    static uint16_t elapsed_x_pos = 0, progress_bar_solid_width = 0;
    static char elapsed_string[16];
    #if ENABLED(SHOW_REMAINING_TIME)
      static uint16_t estimation_x_pos = 0;
      static char estimation_string[10];
      #if BOTH(DOGM_SD_PERCENT, ROTATE_PROGRESS_DISPLAY)
        static uint16_t progress_x_pos = 0;
	...
	*/
    ...
    // Progress / elapsed / estimation updates and string formatting to avoid float math on each LCD draw
      ...
      if (p != lastProgress) {
        ...
        progress_bar_solid_width = uint16_t((PROGRESS_BAR_WIDTH - 2) * progress / (PROGRESS_SCALE) * 0.01f);
  ...
  //if (PAGE_UNDER(6 + 1 + 12 + 1 + 6 + 1))
  {
	...
	*/
	...
	{ //EXTRAS_2_BASELINE
	  ...
	  int y = EXTRAS_2_BASELINE + 10;
      drawBox(x, y, 16, 16);     // 42, 42
      // Right edge
