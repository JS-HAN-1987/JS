...
#define DELAY7        \
 ...
 // Arduino Mega, ADK, etc.
	...
	#define write8inline(d) {                                              \
		PORTH = (PORTH & B10000111) | (((d) & 3) << 5) | (((d) & 0xc0) >> 3); \
		PORTE = (PORTE & B11000111) | (((d) & 0xc) << 2) | (((d) & 0x20) >> 2); \
		PORTG = (PORTG & B11011111) | (((d) & 0x10) << 1); \
		WR_STROBE; }
	...
	#define setWriteDirInline() {                                   \
		DDRE |=  B00111000;  DDRG |= B00100000;  DDRH |= B01111000; }
	...
	#define write8inline(d) {                                              \
		PORTL = (PORTL & B00001111)|(((d)&B11110000) << 0); \
		PORTK = (PORTK & B11100001)|(((d)&B00001111) << 1); \
		WR_STROBE; }
	#define read8inline(result) {                                      \
		RD_ACTIVE;                                                       \
		DELAY7;                                                          \
		result = ((PINL & B11110000) >> 0) | ((PINK & B00011110) >> 1);  \
		RD_IDLE; }
	#define setWriteDirInline() {                                   \
		DDRL |=  B11110000; DDRK |=  B00011110; }
	#define setReadDirInline()  {                                   \
		DDRL &= ~B11110000; DDRK &= ~B00011110; }
